...
(function () {
  ...
  // click-to-reveal cover with no payment involved. It is generated by its
  // own paid call to Grok now (lib/prompts.js's PREMIUM_SCHEMA), so
  // `state.profile.premiumAnalysis` holds the result once there is one — its
  // mere presence *is* "unlocked"; there is no separate boolean that could
  // drift out of sync with whether real content actually exists. A second
  // paid section, "Supplementary analysis", briefly sat behind this same
  // unlock beside the roast — that section is gone, and this call is the
  // roast alone again.
  //
  ...
  function bonusBlock() {
    ...
    return '<div class="card section-card bonus-card">' +
      ...
      (analysis ? bonusBodyHtml(analysis) : '') + '</div></div>';
  }
  ...
  /** Fills the card's body once a real result has arrived. */
  function revealRoast(analysis) {
    ...
    body.innerHTML = bonusBodyHtml(analysis);
    body.hidden = false;
    cover.hidden = true;
