// The Claude calls behind Kindred's two analyses.
//
...
const MODEL = process.env.KINDRED_MODEL || 'claude-opus-5';
...
async function complete(params) {
  ...
  try {
    return { data: JSON.parse(raw), usage: message.usage, model: message.model };
  } catch (error) {
    throw new ClaudeError('Claude returned a response that could not be read as JSON.', 502);
  }
}

/** Full personality profile from an Instagram evidence digest. */
function analyseProfile(digest) {
  return complete({
    system: prompts.PROFILE_SYSTEM,
    schema: prompts.PROFILE_SCHEMA,
    user: 'Here is the Instagram evidence digest for one person. Analyse it and produce their profile.\n\n' +
      '<evidence>\n' + JSON.stringify(digest) + '\n</evidence>',
  });
}

/** Romantic and platonic compatibility from two shareable cards. */
function analyseCompatibility(a, b) {
  return complete({
    system: prompts.COMPATIBILITY_SYSTEM,
    schema: prompts.COMPATIBILITY_SCHEMA,
    user: 'Assess how these two people would work together, romantically and platonically.\n\n' +
      '<person_a>\n' + JSON.stringify(a) + '\n</person_a>\n\n' +
      '<person_b>\n' + JSON.stringify(b) + '\n</person_b>',
  });
}
...
module.exports = { analyseProfile, analyseCompatibility, describeError, ClaudeError, MODEL };
