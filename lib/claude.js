...
// The paid call's own model, independent of MODEL — Sonnet 5 rather than
// whatever the free report's Claude fallback uses. It measured noticeably
// cheaper and faster than Opus at the same effort in the Cost section's own
// table (roughly 40% less on both input and output), which is what makes
// affording `high` effort below reasonable for this call specifically.
const PREMIUM_MODEL = process.env.PSYCHEAI_PREMIUM_MODEL || 'claude-sonnet-5';
 ...
 * report uses.
 *
 * The paid call used to run at `medium` on Opus, purely for latency: four
 * sections from a ~45,000-token digest with thinking at `high` measured past
 * five minutes of wall clock, and unlike the free report, the reader is
 * watching it having *already paid* — the worst place in the app to make
 * somebody wait. Moving the paid call to Sonnet 5 (see PREMIUM_MODEL) is what
 * makes putting effort back to `high` affordable: at the same effort Sonnet
 * runs at 60% of Opus's input rate and 60% of its output rate, which is a
 * large enough gap that `high` on Sonnet is expected to cost no more than
 * `medium` did on Opus, quite possibly less — though the exact number depends
 * on tokens actually spent thinking, which Anthropic does not publish a fixed
 * ratio for per effort level, so this is a reasoned expectation, not a
 * measured one. See the Cost section in the README for the worked numbers.
 *
 * Sonnet's own speed does not fully cancel `high` costing more wall clock
 * than `medium` did — the reader may still wait several minutes. Set
 * PSYCHEAI_PREMIUM_EFFORT=medium to trade some of that quality back for
 * latency again, same as before. The free report's own effort is
 * PSYCHEAI_EFFORT.
 */
const EFFORT = process.env.PSYCHEAI_EFFORT || 'high';
const PREMIUM_EFFORT = process.env.PSYCHEAI_PREMIUM_EFFORT || 'high';
...
/**
 * One group of the paid pass — see PREMIUM_GROUPS in lib/prompts.js. The four
 * paid sections are three independent calls now, run concurrently by the
 * route, so the reader waits for the longest group rather than the sum of all
 * four. Each carries the whole system prompt and only its own slice of the
 * schema.
 */
function analysePremiumGroup(digest, group) {
  return complete({
    system: group.system,
    schema: group.schema,
...
/**
 * The whole paid pass, as one promise, for callers that want it that way —
 * the groups still run concurrently underneath. The streaming route does not
 * use this (it wants each group the moment it lands, not all of them at the
 * end); it exists so that "run the paid analysis" remains one call for
 * anything that does not care about progressive delivery.
 */
async function analysePremium(digest) {
  const results = await Promise.all(
    prompts.PREMIUM_GROUPS.map(group => analysePremiumGroup(digest, group)));
  return {
    ...
    // One group falling back to an unconstrained call is enough to say the
    // whole pass lost the guarantee — reporting it per group would let a
    // caller read `true` off a pass that was only partly enforced.
    constrained: results.every(result => result.constrained),
  };
}
...
function describeError(error) {
  ...
  if (error instanceof Anthropic.PermissionDeniedError) {
    // Not naming MODEL specifically: this file now requests two different
    // models (MODEL for the free report's Claude fallback, PREMIUM_MODEL for
    // the paid call), and this function has no way to know which one a given
    // error came from — naming the wrong one would be worse than naming
    // neither.
    return { status: 500, message: 'This API key does not have access to the requested model.' };
  }
