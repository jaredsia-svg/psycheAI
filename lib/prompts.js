...
const WELLNESS_DIMENSIONS = [
  ['lifeTrajectory', 'Where this person appears to be in their life and which way it has been moving, read across the whole span of the export rather than from the last few months. What phase the data suggests — studying, building something, settling, changing direction, holding steady — and whether the arc across years reads as ascending, plateaued, turbulent or hard to call. Read their apparent satisfaction from what they actually say and do about their own life: what they celebrate, what they stop mentioning, what they return to. Flatter or quieter stretches are worth naming *as stretches* — a period where the posting thinned, a thread that stopped, a subject that disappeared for a year — with the honest note that an export cannot tell you why, and that people go quiet for as many good reasons as bad. Never describe such a stretch as a depressive episode, a breakdown, a crisis or any other clinical event, and never imply you know what happened in it.'],
  ['outlook', 'Whether they write as though things tend to work out or tend not to, from their own words rather than from a guess at their temperament. Read how they frame the future against the past: are plans stated as expectation or as hope, are setbacks written as temporary and specific or as permanent and general, does good news get claimed or hedged, is the tone forward-looking or retrospective. Weigh what they wrote in their own captions and messages far above what they reposted. Two honest cautions: a feed is partly a performance, so relentless positivity may be presentation rather than outlook, and a dry or deadpan register is a style, not pessimism. This is a reading of how somebody writes about their life, never a statement about their mood, their mental state, or how they actually feel — and despair, hopelessness and the like are words you may not apply to a person here, however bleak the writing.'],
...
const PROFILE_SCHEMA = object({
  ...
  summary: { type: 'string', description: 'Two or three tight paragraphs, separated by blank lines, written to them as "you". This is the whole report in miniature, so land the findings from every section below — the MBTI type, where they sit high and low on the Big Five, what they care about, how they are to be close to, and how they work — as one flowing portrait rather than a list. Name the type and the traits explicitly so a reader who stops here still knows the answers. Do not re-explain the character above, and do not contradict any section below. If photographs came through and they show something the words do not — where they spend their time, whether they are usually alone or in company, how much care goes into what they publish, or how any of that has shifted across the years — give it one or two sentences here, inside the photograph rules at the end of this prompt. If they only confirm what the text already established, leave them out and write the paragraph you would have written anyway: a sentence about pictures that adds nothing is worse than no sentence at all.' },
  bigFive: object({
...
const PREMIUM_SCHEMA = withDefs({
  ...
  //
  // **The two newest are the closest this app has come to the line it will
  // not cross**, and they are the reason the hard limits below grew rather
  // than stayed as they were. "Where are they in life, and were there low
  // periods" and "hopeful or in despair" are, worded slightly differently,
  // requests for a mood reading and an episode history. What the export can
  // actually support is narrower and is what the field descriptions ask for:
  // an arc across years, a stretch where the posting thinned, and how
  // somebody writes about their own future. `outlook` is named for the
  // writing rather than the feeling for the same reason `physicalActivity`
  // was: the narrower name is what stops the description drifting back
  // towards the wider claim.
  //
...
const PREMIUM_SYSTEM = `You are PsycheAI, writing the paid half of a report whose free half is already written. Write for someone who has that free report in hand and already knows their character, their Big Five scores, their MBTI type and how they are described at work: do not repeat it, summarise it or re-derive it.\n\nYou write four sections, in this order, and they are four different jobs rather than four angles on one. **wellness** is a careful behavioural read of six dimensions with suggestions attached. **attachment** is a single guess about closeness, shown working. **careerAssessment** is a coach deciding what this person should do next. **harsh** and **advice** are the roast — the one place in the whole report written to be accurate rather than kind. The register changes completely between the third and the fourth, and it must: a reader who paid for all four should not find the roast's tone leaking into the wellness read, which is the section with the most reason to be careful, or the wellness read's hedging draining the roast, which is the section they most obviously paid for.
...
These three are written in the free report's voice — second person, warm but not sycophantic, specific, honest about uncertainty. They are behind a paywall because they are the most valuable part of the report, not because they are the harshest; nothing about being paid for changes how carefully they are written.
...
**Do not read a mood.** \`outlook\` reads how somebody writes about their own life — whether setbacks are framed as temporary or permanent, whether plans are stated as expectation or hope. It does not diagnose how they feel, and a quiet stretch is not sadness any more than a burst of posting is happiness. You may describe the writing; you may not describe the person's inner state. **"Despair", "hopeless", "spiralling", "in a dark place" and their synonyms are not available to you here** — they read as clinical claims however they are meant, and the register they belong to is the one this section exists to stay out of.

**A low stretch is a stretch, not an episode.** \`lifeTrajectory\` may name a period where the posting thinned, a thread stopped or a subject vanished for a year, because those are things the export actually shows. It may not say what happened in that period, and it may never label it a depressive episode, a breakdown, a crisis, a relapse or any other clinical event. People go quiet for as many good reasons as bad — a new job, a new baby, a deliberate break from the app — and an export cannot distinguish them. Name the gap, say plainly that its cause is not visible here, and stop.
...
function premiumBlocks(digest) {
  return [{
    ...
    text: 'Here is the same evidence digest already analysed once for the free report, built from their ' +
      ...
      'read, the career coaching and the roast — the four sections the free report does not carry.\n\n' +
      '<evidence>\n' + JSON.stringify(digest) + '\n</evidence>',
  }];
}

// ---------- splitting the paid pass into parallel calls ----------
//
// The four paid sections used to be one generation, so a reader waited for the
// sum of all four — measured at about eight minutes on Opus and four on Sonnet.
// They are written as three independent calls now, run concurrently, so the
// wait is the longest single group rather than the total. Nothing about what
// each section says changes: every group carries the whole of PREMIUM_SYSTEM,
// including the hard limits, and each one's schema is carved out of
// PREMIUM_SCHEMA rather than rewritten beside it.
//
// **Why three and not two.** A two-way split has to put the boundary
// somewhere, and the only boundary that does not mix registers — the careful
// three against the roast — is badly unbalanced, since the roast is two text
// fields and the other three are the bulk of the output. Wall clock would
// barely move. Three groups keep every call register-pure *and* put the
// largest section, wellness, on its own, which is what actually decides the
// wait.
//
// **Why the full system prompt in every group**, rather than a trimmed one per
// group: the hard limits — no clinical condition named, implied or predicted —
// have to hold on every call, and the roast call is the one with the most
// reason to lose them. Sending the whole prompt three times costs input tokens
// (it is cached, but three concurrent calls race that cache rather than
// sharing it) and buys the guarantee that no group can be missing a rule
// because somebody split the prompt wrong. That trade is deliberate.
const PREMIUM_GROUP_PLAN = [
  ['wellness', ['wellness'],
    'the wellness read alone — the six dimensions, the overall prose and the suggestions'],
  ['relational', ['attachment', 'careerAssessment'],
    'the attachment read and the career assessment — both in the report\'s careful register'],
  ['roast', ['harsh', 'advice'],
    'the roast alone — `harsh` and `advice`, the two fields written to be accurate rather than kind'],
];

/** Every `$defs` name a subtree actually references, so unused ones can be dropped. */
function refsUsed(node, found) {
  const out = found || new Set();
  if (!node || typeof node !== 'object') return out;
  if (Array.isArray(node)) {
    for (const item of node) refsUsed(item, out);
    return out;
  }
  if (typeof node.$ref === 'string') out.add(node.$ref.replace('#/$defs/', ''));
  for (const value of Object.values(node)) refsUsed(value, out);
  return out;
}

const PREMIUM_ALL_DEFS = {
  wellnessDimension: WELLNESS_DIMENSION_DEF,
  point: PREMIUM_POINT_DEF,
  facet: PREMIUM_FACET_DEF,
};
...
/**
 * One group's schema, carved from PREMIUM_SCHEMA rather than written beside
 * it — a second copy of these field definitions is exactly the drift this
 * codebase spends most of its comments preventing.
 *
 * Only the `$defs` a group actually references come with it. An unused
 * definition is not free: the compiled grammar is built from the whole schema,
 * and carrying all three everywhere would rebuild a smaller version of the
 * size problem that forced `$defs` in the first place.
 */
function premiumGroupSchema(fields) {
  const properties = Object.fromEntries(fields.map(field => [field, PREMIUM_SCHEMA.properties[field]]));
  const used = refsUsed(properties);
  const defs = Object.fromEntries(
    Object.entries(PREMIUM_ALL_DEFS).filter(([name]) => used.has(name)));
  const schema = object(properties);
  return used.size ? withDefs(defs, schema) : schema;
}

/**
 * Appended to PREMIUM_SYSTEM for a group. The grammar already prevents the
 * model producing anything outside the group, so this is not what enforces
 * the split — it is what stops the model *apologising* for the omission, or
 * cross-referencing a section it cannot see, having just been told at length
 * that it writes four.
 */
function premiumGroupFocus(describes) {
  return '\n\n# This call\n\nThe four sections above are being written as three separate calls running ' +
    'at the same time, so that the reader waits for the longest rather than the sum. **This call ' +
    'writes ' + describes + '.** The others are already being written by their own calls: do not ' +
    'produce them, do not summarise them, and do not refer to them as something you are about to ' +
    'write or have just written. Everything else above — the evidence rules, the register, and every ' +
    'hard limit — applies to this call in full.';
}

const PREMIUM_GROUPS = PREMIUM_GROUP_PLAN.map(([key, fields, describes]) => ({
  key,
  fields,
  schema: premiumGroupSchema(fields),
  system: PREMIUM_SYSTEM + premiumGroupFocus(describes),
}));
