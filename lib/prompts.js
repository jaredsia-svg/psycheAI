...
const PROFILE_SCHEMA = object({
  ...
  cardHighlights: { type: 'string', description: 'Exactly four sentences that condense the two paragraphs of `summary` above for the shareable card shown at the top of the report — genuine summarizing, in your own words, never sentences copied verbatim out of `summary`. The first two sentences summarize the first paragraph of `summary`; the next two summarize the second paragraph. (If `summary` runs to a third paragraph, it is not covered here — this card only ever holds two paragraphs\' worth.) Keep the same "you" voice and name the same type and traits `summary` already named, but write these four sentences to stand on their own as a short, self-contained portrait — not a trailer that only makes sense once someone has read the fuller version below it.' },
  bigFive: object({
