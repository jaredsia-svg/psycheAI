...
- **Messages per thread** — \`directMessages.totalMessages\` ÷ \`threads\`. Thousands of messages across a handful of threads is *depth*: a few intense close ties, which is textbook introversion however large the total. The same volume spread thin across many threads is breadth, and that is extraversion.
- **Group versus one-to-one** — \`groupThreads\` against \`threads\`. Sustained group-chat presence is genuine extraversion evidence in a way dyadic messaging is not. A person with fifty threads and one group is not a social hub.
...
- **Reach versus circle** — \`followers\` and \`following\` are aspiration and consumption as much as sociability. \`closeFriends\` is a truer number.
