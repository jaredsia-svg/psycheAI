...
const PROFILE_SCHEMA = object({
  ...
  // A second, shorter lens beside MBTI — one type, one wing, one paragraph.
  // No per-facet breakdown: MBTI already carries the long-form typing read,
  // and two of those side by side would be a wall rather than a second look.
  enneagram: object({
    ...
    confidence: { type: 'string', enum: CONFIDENCE_LEVELS },
    why: { type: 'string', description: 'Two or three sentences of evidence from their actual data, naming the core fear and desire this type centres on and where their data shows it.' },
...
- **Enneagram**: a short second lens beside MBTI, not a rephrasing of it — one type (1-9), its usual nickname, and the single adjacent wing if one is clear, left blank rather than forced. State the core fear and desire that type centres on and tie it to something specific in their data; if the Enneagram read and the MBTI read seem to pull in different directions, say so plainly in the caveat rather than smoothing it over. Say plainly that Enneagram is popular rather than empirically validated.
