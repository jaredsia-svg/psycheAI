...
const PROFILE_SCHEMA = object({
  ...
  mbti: object({
    ...
    letters: listOf({
      ...
      strength: { type: 'string', enum: ['slight', 'moderate', 'clear'], description: 'How strongly the data leans this way, read as the *balance* between `why` and `counterEvidence` rather than asserted on its own. `clear` only where the opposing side stayed thin after an honest search for it; `moderate` where the chosen side is the stronger of two real showings; `slight` where the two are close enough that a careful reader could take the other letter. Most people are not extreme on every axis and at least one of the four is usually slight. On E/I, track the Big Five extraversion score you have already written: slight near 50, clear only past roughly 70 or 30.' },
      why: { type: 'string', description: 'Two or three sentences making the case for the letter you chose, citing at least two distinct pieces of evidence drawn from different parts of the digest — not two readings of the same caption — each with a count or a proportion on it. Point at the behaviour, not the conclusion: "thirty-one of the fifty sampled captions name a specific place, price or model" earns its letter, "you are detail-oriented" only restates it.' },
      counterEvidence: { type: 'string', description: 'The strongest case *against* the letter you chose: what somebody who thought they were the opposite type would point at, stated at full strength with a count on it rather than raised in order to be waved away, then a clause on why it did not carry the axis. `strength` is read off this, so it has to be a real search — a four-letter judgement made from one social-media export always has something on the other side, and finding nothing means not having looked. Use the empty string only where an axis is genuinely one-sided, which is rare and should pair with `clear`.' },
...
**On every axis, write \`counterEvidence\` before you settle \`strength\`.** The field exists to stop a letter being chosen on the first thing that pointed at it. Search for the opposing case honestly, state it at its real weight with its own count, and then let the gap between the two sides pick the word: close is \`slight\`, one clearly stronger showing is \`moderate\`, and \`clear\` is only earned where the other side genuinely had little to offer. A \`clear\` on all four axes is a report that did not look.
...
- **MBTI**: give your best type and its usual nickname, then work axis by axis — nothing else. Each axis is an argument with two sides, and you write both: \`why\` makes the case for the letter, citing **at least two separate pieces of evidence from different parts of the digest**, each with a count on it; \`counterEvidence\` makes the best case against it, at full strength and with its own count, and says in a clause why it lost; \`strength\` is then read off the gap between them rather than decided first and justified after. Finish with what the letter looks like in *their* week rather than in a textbook. The per-axis writing carries the whole section, so make it specific: if a sentence would survive being pasted into a stranger's profile, rewrite it or cut it, and let at least one of the four sting slightly. Where an axis is genuinely close, say "slight" and mean it — a hedged letter is more useful than a confident wrong one. There is no summary paragraph; do not write one into the last axis instead.
...
Other people are all over this export without having agreed to any of it — named in captions, written to in messages, listed as friends. Do not describe, count, identify or infer anything whatsoever about them. Say nothing about anyone's race, ethnicity, body, attractiveness, age, gender, wealth or health — not about the reader and not about anyone they mention — and do not use any of it as evidence for any conclusion. Do not read a location precisely enough to place where someone lives or works.
