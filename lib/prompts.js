...
const PROFILE_SCHEMA = object({
  ...
  activity: object({
    ...
    diet: object({
      ...
      topAccounts: listOf({
        account: { type: 'string', description: 'How to name it: for a media outlet, brand, publication, team, shop or public creator, write the handle or name exactly as it appears in the data. For a personal account — someone they know, a friend, a relative, a colleague — do NOT write the handle or any name. Write what it is instead: "a friend you have run with since 2021", "a relative". That person did not agree to appear in anybody\'s personality report, and the reader already knows who they are.' },
        kind: { type: 'string', enum: ['media', 'brand', 'creator', 'community', 'personal'] },
        share: { type: 'string', description: 'What share of their attention this one takes, as a plain phrase with the real number: "about 1 like in 9", "31 comments, more than any other account". Attention here means likes, saves and comments — the export carries no timing data at all, so never write minutes, hours, screen time or "time spent".' },
        why: { type: 'string', description: 'One sentence on what this account being here says about them. Not a description of the account.' },
      }),
      algorithmRead: { type: 'string', description: 'Two or three sentences on the gap between what Instagram\'s own inferred topics and ad interests say this person is, and what their posting and writing actually show. Name the specific inferred topics. This is the platform\'s commercial model of them, not a finding about them — say plainly where it is wrong, and where it has spotted something they have not said out loud.' },
    }),
    ...
    // Advice earns its place here and nowhere else in the report: this is the
    // only section whose findings are about a thing the reader can change
    // tomorrow. Both lists are required — a recommendations block with no
    // anti-recommendations turns into a list of chores, and the anti- side is
    // where the non-obvious answers live.
    recommendations: { ...pointList, description: 'Two or three changes worth making, drawn from anywhere in this whole section — posting, rhythm, trajectory or diet. Each must be small enough to do this week and specific to what their data actually shows.' },
    antiRecommendations: { ...pointList, description: 'One or two things NOT to change: something in their use of the app that looks like a problem, or that the usual advice would tell them to fix, and that is either fine or actively good for them. Name the advice you are contradicting.' },
  }),
  interests: listOf({
    name: { type: 'string' },
  ...
  // The rest of the report is written to be fair. This is written to be
  // accurate without being kind, which is a different thing and is the whole
  ...
  // page says it is not a clinical or diagnostic tool. `trajectory` is the
  // honest version of that question: where the behaviour goes, not what
  // illness it is.
  bonus: object({
    harsh: { type: 'string', description: 'Two or three paragraphs. The least charitable reading of this person that the evidence still fully supports — the version a sharp friend would give after the second drink, with the softening removed. Attack patterns and choices, never the person: nothing about their appearance, body, intelligence, worth or anything they cannot change. It must stay evidence-bound, exactly like every other section — cite the actual captions, counts and rhythms. Unkind and true, never cruel and unfounded. If the data does not support a hard read, say so and write less; an invented insult is worse than a short section.' },
    advice: { type: 'string', description: 'Two or three paragraphs of the advice a friend would give if they were not managing your feelings. Draw on the whole report — relationships, work, values, what they avoid — not on their Instagram habits, which the behaviour section above has already covered; do not repeat those recommendations here. Plain, direct, and specific to them. No self-help register, no affirmations, no "be kind to yourself". Say the thing that is actually hard to hear and then say what to do about it.' },
    trajectory: { type: 'string', description: 'Two paragraphs on where these patterns end up in five years if nothing changes, and what the first thing to break is likely to be. This is a behavioural forecast and nothing else: burnout, drift, resentment, isolation, a career plateau, a relationship that quietly empties out. You must NOT name, imply or hint at any mental or physical health condition, diagnosis, disorder or illness, and you must not use clinical language — no depression, no anxiety disorder, no burnout-as-diagnosis, no "at risk of". You are reading a trend in behaviour, not a medical chart. End with what would change the trajectory.' },
  }),
  card: CARD_SCHEMA,
});
...
- **activity.diet**: what they take in, which the rest of the report barely touches. You are given four separate appetites and they are rarely the same: \`following\` is what they subscribed to, \`mostLikedAccounts\` is what actually catches them, \`mostSavedAccounts\` is what they meant to come back to, and \`mostEngagedWith\` is who they actually talk to. Read the **gaps**. A person following six hundred accounts and engaging with forty is paying for a subscription they stopped reading; a person whose saves are all recipes and whose captions show the same three meals has an ambition that is not converting. Name the top accounts concretely — vague is useless here — and give each one a real share of their attention.

  Two hard rules for this facet. **Attention means likes, saves and comments.** The export contains no timing data whatsoever: no watch time, no session length, no screen time. Never write minutes, hours, "time spent" or anything implying you know how long they looked at something, because you do not. And **name only accounts that are not private individuals**: outlets, brands, shops, teams, publications and public creators by their handle, but a friend or a relative described rather than named. The reader knows perfectly well who their friends are, and a handle written into a report they may hand to somebody else drags in a person who never agreed to any of this.

- **activity.recommendations and activity.antiRecommendations**: the one place in this report you may tell them what to do, because it is the only section about something they can change tomorrow. Advise on the **composition** of what they take in, never on the volume: "the six hundred accounts you never read are what the ranking model learns from" is a finding, "you are on your phone too much" is a lecture, and nothing in this data tells you how long anybody spent anywhere. Every item must trace back to something you actually wrote above it in this section. Ban the whole genre of advice that would fit anybody — digital detoxes, screen-time limits, "curate your feed", "be present" — and never advise them to follow, unfollow, mute or block any specific person. The anti-recommendations matter as much as the recommendations: find the thing that looks like a problem and is not, name the standard advice you are contradicting, and say why they should ignore it.
...
- **bonus**: the rest of the report is written to be fair to them. This section is written to be accurate without being kind, and the reader has to click through a cover to see it, so nobody meets it by accident. That is a licence to drop the softening, not a licence to make things up: every hard line still has to be one the evidence carries, and the section is worth more short and true than long and invented. Go after patterns, choices and self-deceptions — never appearance, body, intelligence, worth, or anything about them that cannot change. \`harsh\` is the least charitable *supportable* reading. \`advice\` is what a friend says when they have stopped managing your feelings, drawn from the whole report rather than from their posting habits, which the behaviour section already handled. \`trajectory\` is where this goes in five years and what breaks first.

  **\`trajectory\` is a behavioural forecast and never a medical one.** Do not name, imply or gesture at any mental or physical health condition, disorder, diagnosis or illness — not depression, not anxiety, not burnout as a condition, not "at risk of" anything clinical. You are reading where a habit leads: drift, resentment, isolation, a plateau, a relationship emptying out. You have someone's Instagram export and no clinical training, no history, no assessment and no standing, and a diagnosis invented from posting patterns would be a confident falsehood about somebody's health in a document they will keep. Read the behaviour and stop there.
