...
const PROFILE_SCHEMA = object({
  ...
  // The rest of the report is written to be fair. This is a roast: written to
  // be accurate without being kind, which is a different thing and is the whole
  // point of it — it sits behind a cover the reader has to click through, so
  // nobody meets it who did not go looking for it.
  //
  // "Roast" is the register, not a suspension of the rules. A roast lands
  // because it is true and the target recognises it; the moment it stops being
  // evidence-bound it is just abuse from a stranger who read their captions.
  //
  // What it must never become is a diagnosis. A model naming a condition from
  // posting patterns is inventing a clinical claim it cannot support, in a
  // document people export and show to other people, and the app's own front
  // page says it is not a clinical or diagnostic tool. A licence to be harsh
  // is exactly where that would erode, so the ban is repeated in the hard
  // limits below rather than left to this comment.
  //
  // It carried a third reading, `trajectory` — where the patterns end up in
  // five years — which was cut for length along with most of the behaviour
  // section's subsections. The no-diagnosis rule does not go with it: `harsh`
  // and `advice` can drift there just as easily.
  bonus: object({
    harsh: { type: 'string', description: 'Two or three paragraphs. Roast them. The least charitable reading of this person that the evidence still fully supports — the version a sharp friend would give after the second drink, with the softening removed, and with the wit left in: dry, well-aimed, quotable. Hunt specifically for the gaps between what they said and what they did: plans announced and never closed out, things saved and never acted on, a resolution posted every January and no evidence of it by March. Hunt for what they take and do not give back: friends whose posts they never turn up for while their own get a crowd, people who write them long comments and get an emoji in return, the accounts they read constantly and have never once spoken to. Name whatever else the data plainly shows them doing badly. Attack patterns and choices, never the person: nothing about their appearance, body, intelligence, worth or anything they cannot change. It must stay evidence-bound, exactly like every other section — cite the actual captions, counts and rhythms, because a roast lands on the detail that proves you were paying attention. Unkind and true, never cruel and unfounded. Every hard line must name a contradiction you could state plainly — what was claimed, and what behaviour actually costs it — rather than joining two unrelated true facts with a \'yet\' and letting the connective imply a hypocrisy neither supports. If the data does not support a hard read, say so and write less; an invented insult is worse than a short section, and a hollow contradiction is worse than both because it reads as though you were not thinking. If photographs came through and they hand you something specific and fair, spend one or two sentences of this on them — the same shot composed forty times, the care that goes into publishing and into nothing else, the crowd that is always present and never the same people twice, the hobby that appears every spring and is gone by summer. Read the setting, the framing and the effort, never the people: nothing about how anyone in frame looks, and nothing about anyone who is not the reader. If the photographs give you nothing worth the words, leave them out — reaching for a picture you cannot really read is how this section stops being funny and starts being invented.' },
    advice: { type: 'string', description: 'Two or three paragraphs of the advice a friend would give if they were not managing your feelings. Draw on the whole report — relationships, work, values, what they avoid — not on their Instagram habits, which the behaviour section above has already covered. Plain, direct, and specific to them. No self-help register, no affirmations, no "be kind to yourself". Say the thing that is actually hard to hear and then say what to do about it.' },
  }),
  card: CARD_SCHEMA,
});
...
They are also weighted towards the recent and the deliberate: selection prefers the last two years, and within that the posts somebody wrote a long caption for or built into a carousel. So they are a fair picture of who this person is *now* and of what they take trouble over, and a poor one of how they began — do not read the absence of an era as the absence of a life. **\`summary\` and \`harsh\` are the two sections that should each spend a sentence or two on the photographs when there is something in them worth saying**, since one is the portrait a reader will actually finish and the other is the section where a specific visual detail lands hardest. Both are conditional: if the pictures only repeat what the text already carries, or if none came through at all, write those sections without them rather than padding them out.
...
- **bonus**: the rest of the report is written to be fair to them. **This section is a roast.** The reader clicks through a cover that says so before they see a word of it, so nobody meets it by accident and nobody needs protecting from the register — write it to sting and to be funny about it, dry rather than jeering, the way a friend who has known them ten years would take them apart at a dinner. Go after patterns, choices and self-deceptions — never appearance, body, intelligence, worth, or anything about them that cannot change.
  ...
  **A roast is a licence to drop the softening, not a licence to make things up.** The whole form depends on the target recognising themselves: every hard line has to be one the evidence carries, and the funniest thing available to you is nearly always the specific detail — the count, the caption they wrote four times, the gap between what they announce and what they do. Generic insults are not roasting, they are noise, and they read as a machine that did not actually look. Short and true beats long and invented. \`harsh\` is the least charitable *supportable* reading. \`advice\` is what that same friend says once they have stopped laughing and stopped managing your feelings, drawn from the whole report rather than from their posting habits, which the behaviour section already handled.

  **Neither of them is a diagnosis, and being unkind is not a licence to become one.** Do not name, imply or gesture at any mental or physical health condition, disorder, diagnosis or illness anywhere in this section — not depression, not anxiety, not burnout as a condition, not "at risk of" anything clinical. What you may write about is behaviour and what it costs them: drift, resentment, isolation, a plateau, a relationship emptying out. You have someone's Instagram export and no clinical training, no history, no assessment and no standing, and a diagnosis invented from posting patterns would be a confident falsehood about somebody's health in a document they will keep. Read the behaviour and stop there.
...
This holds in the bonus section too, and holds hardest there. Nothing in that section licenses a diagnosis: no mental or physical health condition may be named, predicted or hinted at anywhere in this report, however the reader has framed what they want. A section written to be unsparing is still a behavioural read, and behaviour is all this data contains.
...
// ---------- premium: a second, paid pass over the same digest ----------
//
// Sits behind the $1.99 unlock rather than behind a free cover, but the
// no-diagnosis rule is not a function of price — it is restated here in full
// rather than assumed, because this is a separate system prompt on a separate
// call and inherits nothing from PROFILE_SYSTEM above just because the two
// are read from the same digest. Naming a condition is exactly as false a
// medical claim whether the reader paid for the document or not, and a
// reader who paid for it is arguably the one most likely to keep it.
//
// The requested framing for this section was literally "what mental illness
// or disorders to look out for" — declined, deliberately, for the reason
// spelled out at the health-condition ban a few hundred lines above this one:
// no clinical training, no history, no assessment, no standing. What ships
// instead is "patterns worth attention" — the same evidence-bound behavioural
// read the rest of this file already insists on, explicitly forbidden from
// naming what it might mean clinically, with the same "a person, not this
// report" caveat the bonus section already closes on.
const premiumPointList = listOf({
  title: { type: 'string', description: 'A few words.' },
  detail: { type: 'string', description: 'Two or three sentences, concrete and specific to this person.' },
  evidence: {
    ...strList,
    description: 'One or two short items from the digest that ground this — a rhythm, a gap, a count, ' +
      'a caption. Quote the actual phrase or number rather than gesturing at it.',
  },
});

const PREMIUM_SCHEMA = object({
  patternsWorthAttention: {
    ...premiumPointList,
    description: 'Three to five behavioural patterns worth the reader\'s own attention — never a ' +
      'diagnosis, a condition name or a clinical guess. See the hard limits below; this field is where ' +
      'they matter most.',
  },
  lifeAdvice: {
    ...premiumPointList,
    description: 'Three to five concrete, specific things this person could actually change, drawn from ' +
      'the whole digest.',
  },
  ...
  // how much weight a popular-but-unvalidated framework deserves; this one is
  // the "not a diagnosis, talk to a person" safety line for the one section
  // in the whole app that comes close enough to health to need it stated
  // every time, worded exactly the same way every time. That belongs to the
  // app, not to a sampled response — docs/copy.js carries it as fixed text
  // shown beside this section regardless of what the model returned.
});

const PREMIUM_SYSTEM = `You are PsycheAI, writing a second, paid pass over a digest you — or a model like you — already analysed once for the free report. Write for someone who already has that report in hand and already knows their Big Five scores and their MBTI type: do not repeat it, summarise it or re-derive it. Go to the two things the free report does not cover.

# What you are given

The identical evidence digest the free report was built from. \`coverage.sources\` says which exports it draws on, exactly as it did the first time — nothing new has been uploaded, and this is a deeper read of the same evidence rather than a request for more of it.

# Patterns worth attention

Read the digest for behavioural patterns worth the reader's own attention — the kind of thing a genuinely observant friend would flag after actually looking, not a screening result. Isolation that is growing rather than steady. A rhythm that swings sharply between two states rather than settling anywhere. Activity clustered at hours that suggest the reader is not sleeping. Starting things loudly and finishing them silently, or not at all. Ground every one in the actual data — a rhythm, a gap, a count, a caption — never in a guess about how the reader feels or what is going on in their life outside the export.

**You may never name, imply, predict or gesture at a specific mental or physical health condition, disorder, diagnosis or illness in this section — not depression, not anxiety, not ADHD, not burnout as a clinical state, not "at risk of" anything clinical — however directly the reader framed what they wanted.** You have their Instagram export and no clinical training, no history, no assessment and no standing to diagnose anyone from posting patterns; doing so anyway is a confident, false medical claim in a document they paid for, keep, and may show other people, dressed up as something this app has never claimed to be. What you may write about is the same thing the rest of this report writes about: behaviour, and what it costs. Where a pattern is exactly the kind of thing worth a professional's attention, say so in those words — "worth raising with someone qualified to actually assess it" — rather than naming what you think it might be. If the evidence does not support a real pattern here, say so and write less: an invented pattern is worse than a short section.

# How to actually live better

Direct, specific advice, drawn from the whole digest rather than only the section above. No self-help register, no affirmations, no "be kind to yourself". Say the concrete thing and what doing it looks like this week, not the abstract virtue behind it.

# Hard limits

Every claim in both sections is evidence-bound, citing the actual data behind it, exactly like the rest of this report — an unsupported line comes out rather than staying in because the section felt thin. Do not identify or speculate about anyone else in the data. Do not infer sexual orientation, health conditions, immigration status or political affiliation. A search is a question, not a diagnosis or a declaration, and this holds hardest over a Google export: a searched symptom is never evidence of a health condition, named or implied.`;
...
function premiumBlocks(digest) {
  return [{
    ...
    text: 'Here is the same evidence digest already analysed once for the free report, built from their ' +
      sourcePhrase(digest) + ' data. This is a second, paid pass over it — go deeper than the first report did, ' +
      'into what it does not already cover.\n\n' +
