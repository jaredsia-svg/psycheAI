...
//
// Sits behind a single $1.99 unlock that also covers the roast (moved here
// from the free report — see the comment above PREMIUM_SCHEMA below) and the
// report download. The no-diagnosis rule is not a function of price — it is
// stated in full within this prompt rather than assumed, because this is its
// own system prompt on its own call and inherits nothing from PROFILE_SYSTEM
// above just because the two are read from the same digest. Naming a
// condition is exactly as false a medical claim whether the reader paid for
// the document or not, and a reader who paid for it is arguably the one most
// likely to keep it.
//
// The requested framing for patternsWorthAttention was literally "what
// mental illness or disorders to look out for" — declined, deliberately, for
// the reason spelled out at the health-condition ban a few hundred lines
// above this one: no clinical training, no history, no assessment, no
// standing. What ships instead is "patterns worth attention" — the same
// evidence-bound behavioural read the rest of this file already insists on,
// explicitly forbidden from naming what it might mean clinically, with the
// same "a person, not this report" caveat the roast below already closes on.
...
// as everything else and sitting behind a click-to-reveal cover rather than a
// payment. It moved here — same content, same rules, same "harsh"/"advice"
...
// digest, same as patternsWorthAttention and lifeAdvice beside it.
const PREMIUM_SCHEMA = object({
  ...
  // the "not a diagnosis, talk to a person" safety line for the one call in
  // the whole app that comes close enough to health to need it stated every
  // time, worded exactly the same way every time. That belongs to the app,
  // not to a sampled response — docs/copy.js carries it as fixed text shown
  // beside both the roast and patternsWorthAttention, regardless of what the
  // model returned.
});

const PREMIUM_SYSTEM = `You are PsycheAI, writing a second, paid pass over a digest you — or a model like you — already analysed once for the free report. Write for someone who already has that report in hand and already knows their Big Five scores and their MBTI type: do not repeat it, summarise it or re-derive it. Go to the things the free report does not cover.
