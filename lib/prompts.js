...
const PROFILE_SCHEMA = object({
  ...
  essence: object({
    noun: { type: 'string', description: 'A single noun — one or two words at most — that captures this person. Reach past the obvious: an occupation, an animal, a natural feature, a material, an element, a tool, a weather pattern, a kind of place. "The Lighthouse", "Riverbed", "Sheepdog", "Copper", "Harbour", "Cartographer". Not an adjective, not a personality label, and never an MBTI type.' },
    icon: { type: 'string', description: 'Exactly one emoji character depicting that noun, and nothing else — no words, no punctuation, no variation text.' },
    why: { type: 'string', description: 'Two or three sentences on why this noun and not a neighbouring one, tied to specific things in their data. Make the metaphor earn itself.' },
  }),

  summary: { type: 'string', description: 'Two or three tight paragraphs, separated by blank lines, written to them as "you". This is the whole report in miniature, so land the findings from every section below — the MBTI type, where they sit high and low on the Big Five, what they care about, how they are to be close to and their attachment read, and how they work — as one flowing portrait rather than a list. Name the type and the traits explicitly so a reader who stops here still knows the answers. Do not re-explain the noun above, and do not contradict any section below.' },
  bigFive: object({
  ...
  mbti: object({
    ...
    portrait: { type: 'string', description: 'One paragraph — six sentences at the outside — written to them as "you", on what being this type actually feels like from the inside: where it serves them, where it costs them, and how they are commonly misread. Specific to them, not a copied type description. Do not pad it to length.' },
  ...
  // what the following-and-liking says about where their attention goes.
  activity: object({
    ...
    attention: object({
      headline: { type: 'string', description: 'A few words on what they follow and consume.' },
      detail: { type: 'string', description: 'Two or three sentences on the shape of who they follow and like most, and how far it matches what they post about themselves.' },
    }),
    implications: listOf({
      observation: { type: 'string', description: 'One specific thing in the data. Cite the number where there is one.' },
      implication: { type: 'string', description: 'One or two sentences on what it suggests about them — hedged honestly.' },
    }),
...
- **MBTI**: this section should be the one they send to a friend, so make it earn that. Give your best type and its usual nickname; for each axis, say how strongly the data leans and what that letter looks like in *their* week rather than in a textbook. The **portrait** carries the rest in a single tight paragraph: what being this type feels like from the inside, where it serves them, where it costs them, and how they get misread — written to them and grounded in things you can point to in their data. It should sting slightly in at least one place. Nothing in it may be filler that would be true of every INFP alive: if a sentence would survive being pasted into a stranger's profile, rewrite it or cut it. Brevity is the point here — six good sentences beat twelve padded ones. Where an axis is genuinely close, say "slight" and mean it — a hedged letter is more useful than a confident wrong one.

- **activity**: read the account as behaviour, not statistics. The histograms, the month-by-month series, the ratio of what they publish to what they like and save, the shape of who they follow — these are the parts of the export people never look at themselves, and they are often the most revealing thing in it. Cite real numbers. Say what changed and when. In **implications**, pair one concrete observation with one honest inference, and hedge where the evidence is thin — "you post almost entirely between 6 and 8am" is a fact, "you are a morning person whose day is spoken for by nine" is an inference, and the reader should be able to tell which is which. Be careful not to moralise about screen time.
...
- **essence**: pick the noun before you write anything else, then let the rest of the profile agree with it. A good one is concrete, slightly surprising, and survives being read back to them — "Riverbed" for someone whose life shapes itself around other people's currents, "Kiln" for someone who runs hot and produces. A bad one is a compliment in a costume ("Diamond", "Star") or a restatement of their hobby. If the evidence is thin, pick something honest and modest rather than something grand.
...
function compatibilityBlocks(a, b) {
  return [{
    type: 'text',
    text: 'Assess how these two people would work together, romantically and platonically.\n\n' +
      '<person_a>\n' + JSON.stringify(a) + '\n</person_a>\n\n' +
      '<person_b>\n' + JSON.stringify(b) + '\n</person_b>',
  }];
}
