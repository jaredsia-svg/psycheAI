...
async function createPaymentIntent(description, productName) {
  ...
  if (MOCK) {
    ...
    // The amount rides along with the id in mock mode, because mock mode has
    // no Stripe to retrieve it back from later — see retrievePaymentIntent.
    mockIntents.set(id, product.cents);
    return {
...
async function retrievePaymentIntent(id) {
  if (MOCK) {
    ...
    return {
      ...
      amount: mockIntents.get(String(id)), currency: CURRENCY,
    };
  }
  if (!SECRET_KEY) {
    throw Object.assign(new Error('Payments are not configured on this server.'), { status: 503 });
...
module.exports = {
  ...
  UNLOCK_PRICE_CENTS, ANALYSIS_PRICE_CENTS, PRODUCTS, CURRENCY, COUNTRY,
