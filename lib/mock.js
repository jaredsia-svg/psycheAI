...
function analyseProfile(digest) {
  ...
  const data = {
    ...
    // Four sentences, two per paragraph of `summary` above — a real
    // condensation, not the first sentences repeated. Distinct wording from
    // `summary` itself so a check comparing the two catches a version that
    // regressed back to quoting it verbatim.
    cardHighlights: 'Mock card summary, sentence one — a condensed take on the ENFJ read and the ' +
      'standout traits. Mock card summary, sentence two — the organising-without-being-asked ' +
      'pattern, condensed. Mock card summary, sentence three — a condensed take on the giving-more ' +
      'pattern. Mock card summary, sentence four — closing what you open, condensed.',
    bigFive: {
