...
// The paid second pass. Shaped exactly like the real PREMIUM_SCHEMA output —
// two evidence-bearing lists, no caveat field — so the mock flow exercises
// the same rendering path a real call does. The safety caveat itself is
// static copy shown by the client regardless of what this returns, not part
// of this schema at all. Never a condition name here either: the mock
// content is what a fixture-writer would put in the field, and this field's
// whole point is that nothing clinical belongs in it.
function analysePremium(digest) {
  const data = {
    patternsWorthAttention: [
      cited('Long quiet stretches between bursts of posting'),
      cited('Comments given generously, rarely received back'),
      cited('Plans announced publicly, closed out privately or not at all'),
    ],
    lifeAdvice: [
      cited('Pick one open loop from the last month and close it this week'),
      cited('Say the thing to the person instead of posting around it'),
      cited('Protect one evening a week with no phone in the room'),
    ],
  };
  return Promise.resolve({ data, usage: { inputTokens: 0, outputTokens: 0 }, model: 'mock' });
}
