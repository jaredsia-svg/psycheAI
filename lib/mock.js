...
// Only for PREMIUM_GROUPS' field lists — the mock slices its own canned object
// by the same grouping the real call splits on, rather than keeping a second
// list of which section belongs to which group.
...
/**
 * One group of the paid pass, mocked. Sliced out of the same object
 * analysePremium returns rather than written separately, so the mock cannot
 * drift from itself: a group's mock content is by construction exactly what
 * the whole-pass mock would have put in those fields.
 *
 * PSYCHEAI_MOCK_GROUP_MS staggers the groups, so the UI suite can prove
 * sections really do appear one at a time rather than all at once at the end.
 * The real thing staggers on its own, because the three calls genuinely
 * finish at different moments; the mock answers instantly and would otherwise
 * make a progressive reveal indistinguishable from a batch one.
 */
async function analysePremiumGroup(digest, group) {
  // One group failing while the others succeed is the case worth being able
  // to reproduce on demand: it is the only path that reaches the partial
  // result, and it is a path about somebody's money. Named groups rather than
  // a boolean, so a test can pick which one dies.
  if ((process.env.PSYCHEAI_MOCK_FAIL_GROUP || '').split(',').includes(group.key)) {
    throw new Error('Mock failure injected for the ' + group.key + ' group.');
  }
  const whole = await analysePremium(digest);
  const delay = Number(process.env.PSYCHEAI_MOCK_GROUP_MS) || 0;
  if (delay) {
    const step = prompts.PREMIUM_GROUPS.findIndex(candidate => candidate.key === group.key);
    await new Promise(resolve => setTimeout(resolve, delay * (step + 1)));
  }
  const data = Object.fromEntries(group.fields.map(field => [field, whole.data[field]]));
